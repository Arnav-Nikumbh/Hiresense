...
async def chat(req: ChatRequest):
    # Always reload .env fresh in case it was edited after startup
    from dotenv import load_dotenv
    load_dotenv(Path(__file__).resolve().parent.parent / ".env", override=True)
    ...
    if not gemini.is_available():
        ...
        if not key:
            detail = "GEMINI_API_KEY is empty in your .env file."
        elif key == "your_gemini_api_key_here":
            detail = "GEMINI_API_KEY is still the placeholder. Replace it with your real key."
        else:
            detail = f"GEMINI_ENABLED is set to false in your .env file."
        return JSONResponse({"reply": f"⚠️ {detail}", "error": True})
    try:
        ...
        return JSONResponse({"reply": f"❌ Gemini error: {str(e)}", "error": True})

@app.get("/api/debug")
async def debug():
    """Quick diagnostic — shows what the server actually sees."""
    from dotenv import load_dotenv
    load_dotenv(Path(__file__).resolve().parent.parent / ".env", override=True)
    key = os.environ.get("GEMINI_API_KEY", "")
    return {
        "env_file_exists": (Path(__file__).resolve().parent.parent / ".env").exists(),
        "api_key_set":     bool(key) and key != "your_gemini_api_key_here",
        "api_key_preview": key[:8] + "..." if len(key) > 8 else f"[{len(key)} chars]",
        "model":           os.environ.get("GEMINI_MODEL", "not set"),
        "gemini_enabled":  os.environ.get("GEMINI_ENABLED", "not set"),
